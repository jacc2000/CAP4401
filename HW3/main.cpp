...
void histogramEqualizationWithThreshold(Mat& src, int threshold) {
    // Create a mask for dark regions (pixels with intensity less than the threshold)
    Mat mask = src < threshold;

    // Create a copy of the source image where bright regions are set to 0
    Mat srcForEqualization;
    src.copyTo(srcForEqualization);
    srcForEqualization.setTo(0, ~mask);

    // Apply histogram equalization to the modified image (dark regions only)
    Mat histEqualized;
    cv::equalizeHist(srcForEqualization, histEqualized);

    // Combine the equalized dark regions with the original bright regions
    histEqualized.copyTo(src, mask);
}

void detectAndDecodeQR(Mat &src, string &outputImageName) {
    QRCodeDetector qrDecoder;
    vector<Point> points;
    Mat straight_qrcode;

    string data = qrDecoder.detectAndDecode(src, points, straight_qrcode);
    if (data.empty()) {
        cout << "QR Code not detected" << endl;
    } else {
        cout << "Decoded Data: " << data << endl;

        // If a QR code is detected, draw a rectangle around it and put text above the rectangle
        if (points.size() > 0) {
            for (size_t i = 0; i < points.size(); i++) {
                line(src, points[i], points[(i+1) % points.size()], Scalar(0, 255, 0), 4);
            }

            Point top = *min_element(points.begin(), points.end(), [](const Point &lhs, const Point &rhs) {
                return lhs.y < rhs.y;
            });

            // Put the decoded text above the QR code
            putText(src, data, Point(top.x, top.y - 10), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 0), 2);
        }

        // Save the output image with the text and rectangle on it
        if (!outputImageName.empty()) {
            imwrite(outputImageName, src);
        }
    }
}

void equalizeHistogramChannel(Mat& src, bool equalizeR, bool equalizeG, bool equalizeB) {
    vector<Mat> channels(3);
    split(src, channels);

    if (equalizeR) {
        equalizeHist(channels[2], channels[2]); // Equalize the Red channel
    }
    if (equalizeG) {
        equalizeHist(channels[1], channels[1]); // Equalize the Green channel
    }
    if (equalizeB) {
        equalizeHist(channels[0], channels[0]); // Equalize the Blue channel
    }

    merge(channels, src);
}

void convertRGBtoHSV(Mat& src) {
    // Convert from RGB to HSV color space
    cvtColor(src, src, COLOR_RGB2HSV);
}

void equalizeVComponent(Mat& src) {
...
void equalizeHComponent(Mat& src) {
    vector<Mat> hsvChannels(3);
    split(src, hsvChannels);
...
void process_line(const string& line) {
    ...
    while (ss.good()) {
        ...
        if (function_name == "histequal") {
            if (src.channels() > 1) cvtColor(src, src, COLOR_BGR2GRAY);
            // Apply histogram equalization to the entire image
            Mat histEqualized;
            cv::equalizeHist(src, histEqualized);
            src = histEqualized;

        } else if (function_name == "histequalThresh") {
            if (src.channels() > 1) cvtColor(src, src, COLOR_BGR2GRAY);
            int threshold;
            ss >> threshold; // Read the user-provided threshold
            histogramEqualizationWithThreshold(src, threshold);

        } else if (function_name == "detectQR") {
            
            string outputImageName;
            ss >> outputImageName;
            detectAndDecodeQR(src, outputImageName);

        } else if (function_name == "hisEqColor") {
            int eqR, eqG, eqB;
            ss >> eqR >> eqG >> eqB; // Flags for equalizing R, G, B components
            equalizeHistogramChannel(src, eqR, eqG, eqB);

        } else if (function_name == "convertto_hsv") {
            convertRGBtoHSV(src);
            save_image(src, tgt_name);
        } else if (function_name == "histequal_V_RGB") {
            equalizeVComponent(src);
            save_image(src, tgt_name);
        } else if (function_name == "histequal_H") {
            equalizeHComponent(src);
            save_image(src, tgt_name);
        } else if (function_name == "histequal_S") {
            equalizeSComponent(src);
            save_image(src, tgt_name);
        } else if (function_name == "histequal_V") {
            vector<Mat> hsvChannels(3);
            split(src, hsvChannels);
            equalizeHist(hsvChannels[2], hsvChannels[2]);
            // Convert V channel to grayscale image for display
            Mat V_channel_display;
            hsvChannels[2].convertTo(V_channel_display, CV_8U);
            save_image(V_channel_display, tgt_name+"_V_channel.png");
            // Merge and convert back to RGB
            merge(hsvChannels, src);
            cvtColor(src, src, COLOR_HSV2RGB);
            save_image(src, tgt_name);
        
        } else if (function_name == "hisStretch") {
